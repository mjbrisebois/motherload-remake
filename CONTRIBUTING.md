...
`TileType` is intentionally numbered to match Phaser tilemap GIDs:

- 0 EMPTY, 1 DIRT, 2 ROCK, 3 COPPER, 4 IRON, 5 SILVER, 6 GOLD, 7 PLATINUM, 8 RUBY, 9 DIAMOND, 10 LAVA

The tilemap tileset uses `firstgid=1`, so data value `0` renders nothing and `1..10` index the strip. Tile graphics are generated procedurally in `generatePlaceholderTextures()` from `TILE_META` — no asset files.
...
1. Rock probability ramps with depth.
2. Lava probability ramps with depth (starts at row 60+ underground).
3. Cumulative ore probability based on per-ore depth-band triangular distribution.
4. Fallback to dirt.

Tests assert: deterministic for same seed, no lava in shallow rows, no diamond near surface, sanity counts of every type across the full depth.
...
| `WORLD_COLS` / `WORLD_ROWS` | 50 / 200 | Underground extent |
